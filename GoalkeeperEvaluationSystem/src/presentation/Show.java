...
public class Show implements Simulation {
    ...
    public String goalkeepersMean(Team team){
        ...
        for(int i = 0; i < team.defenseMean().size(); i++){
            goalkeepersMean = goalkeepersMean.concat(String.format(
                    "%-20s: %.3f%%\n",
                    team.getGoalkeepers().get(i).getName(),
                    team.defenseMean().get(team.getGoalkeepers().get(i).getName()) * 100
            ));
        }
    ...
    public ArrayList<String> getStrikerNames() {
